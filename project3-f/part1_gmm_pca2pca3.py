...
gmm = GaussianMixture(n_components=K, covariance_type=cov_type, n_init=reps, 
                      tol=1e-3, reg_covar=1e-3, init_params=initialization_method).fit(Z_pc_n)
cls = gmm.predict(Z_pc_n)    
...
# Plot results:
figure(figsize=(14,9))
clusterplot(Z_pc_n, clusterid=cls, centroids=cds, y=y, covars=covs)
show()
